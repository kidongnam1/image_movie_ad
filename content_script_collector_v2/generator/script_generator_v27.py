...
def generate(product: str, **kwargs: Any):
    ...
    product_url=str(kwargs.pop("product_url","") or "")
    ...
    data["input_assets"]={"product_image":product_image,"image_available":bool(product_image),"product_url":product_url,"image_usage":"대본 생성에서는 참조 메타데이터로 보존하고 Creative Package에서는 product_image로 전달합니다.","url_policy":"상품 페이지 URL은 참조 메타데이터로만 저장하며 페이지 이미지를 자동 스크래핑하지 않습니다."}
    return data
...
def render_md(data:dict[str,Any])->str:
    ...
    return text+f"\n\n## V2.7 Creative Registry\n- 등록된 A/B/C: {r.get('registered_candidates',0)}개\n- 광고명에 `CR-...` ID를 포함하세요.\n\n## 입력 상품 자산\n- 상품 페이지 URL: `{asset.get('product_url') or '미입력'}`\n- 이미지: `{asset.get('product_image') or '미입력'}`\n"

def parse_args():
    p=argparse.ArgumentParser(description="Script Generator V2.7 with creative registry");p.add_argument("product");p.add_argument("--category",default="");p.add_argument("--features",default="");p.add_argument("--must-emphasize",default="");p.add_argument("--pain-point",default="");p.add_argument("--target",default="일반 소비자");p.add_argument("--description",default="");p.add_argument("--image",default="",help="상품 이미지 경로");p.add_argument("--url",default="",help="상품 페이지 URL 메타데이터");p.add_argument("--intensity",type=int,choices=(1,2,3,4,5),default=4);p.add_argument("--min-score",type=float,default=80);p.add_argument("--outdir",default="outputs");p.add_argument("--require-db",action="store_true");p.add_argument("--performance-db",default=str(DEFAULT_PERFORMANCE_DB));p.add_argument("--performance-file",default="");p.add_argument("--learning-off",action="store_true");p.add_argument("--experiment-min-impressions",type=int,default=1000);return p.parse_args()
def main():
    a=parse_args()
    try:
        d=generate(a.product,category=a.category,features=a.features,must_emphasize=a.must_emphasize,pain_point=a.pain_point,target=a.target,description=a.description,product_image=a.image,product_url=a.url,intensity=a.intensity,min_score=a.min_score,performance_db=a.performance_db,performance_file=(a.performance_file or None),learning=not a.learning_off,experiment_min_impressions=a.experiment_min_impressions)
        if a.require_db and not d["db_integration"]["connected"]:raise RuntimeError("Content DB is not connected or empty.")
        out=Path(a.outdir);out.mkdir(parents=True,exist_ok=True);safe=re.sub(r"[^0-9A-Za-z가-힣_-]+","_",a.product);(out/f"{safe}_script_v2.json").write_text(json.dumps(d,ensure_ascii=False,indent=2),encoding="utf-8");md=out/f"{safe}_script_v2.md";md.write_text(render_md(d),encoding="utf-8");print("Version:",d["version"]);print("Creative registry:",d["creative_registry"]);print("Product image:",d["input_assets"]["product_image"]);print("Product URL:",d["input_assets"]["product_url"]);print("Generated:",md);return 0
