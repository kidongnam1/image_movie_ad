...
echo ==========================================================
echo Script Generator V2.7 - 전체 회귀 + 플랫폼 + 대시보드 + 이미지 검증
echo ==========================================================
echo.

echo [1/12] Python syntax check
python -m py_compile generator\script_generator_v25.py generator\performance_store_v26.py generator\script_generator_v26.py generator\performance_registry_v27.py generator\platform_performance_adapter_v27.py generator\performance_dashboard_v27.py generator\product_image_input_v27.py generator\script_generator_v27.py generator\script_generator_v2.py generator\creative_package_v26.py generator\creative_package_v27.py
if errorlevel 1 goto FAIL

echo [2/12] 기존 selftest + 5상품 V2.5 회귀
python tests\selftest_v2.py
if errorlevel 1 goto FAIL
python tests\test_script_generator_v25.py
if errorlevel 1 goto FAIL

echo [3/12] V2.6 성과 DB / Snapshot Update / 표본축소
python tests\test_ad_performance_learning.py
if errorlevel 1 goto FAIL

echo [4/12] V2.6 학습 재랭킹 / Cold Start / A-B-C
python tests\test_script_generator_v26.py
if errorlevel 1 goto FAIL

echo [5/12] V2.7 Creative Registry 자동등록
python tests\test_script_generator_v27.py
if errorlevel 1 goto FAIL

echo [6/12] V2.7 Meta/TikTok/Naver/Coupang Adapter + Windows CSV encoding
python tests\test_platform_adapter_v27.py
if errorlevel 1 goto FAIL

echo [7/12] V2.7 Standalone Dashboard
python tests\test_performance_dashboard_v27.py
if errorlevel 1 goto FAIL

echo [8/12] V2.7 이미지 파일 / 권리 sidecar / URL 안전검사
python tests\test_product_image_input_v27.py
if errorlevel 1 goto FAIL

echo [9/12] Creative Package V2.6 sidecar + 기존 회귀
python tests\test_creative_package_v26.py
if errorlevel 1 goto FAIL
python tests\test_creative_package_v1.py
if errorlevel 1 goto FAIL

echo [10/12] Creative Package V2.7 Registry + input_assets sidecar
python tests\test_creative_package_v27.py
if errorlevel 1 goto FAIL

echo [11/12] Real Content DB integration + isolated performance DB
...
echo [12/12] Dashboard generation on isolated DB
...
if errorlevel 1 goto FAIL
python -c "from pathlib import Path; p=Path(r'outputs_verify\performance_dashboard_v27.html'); assert p.exists(); t=p.read_text(encoding='utf-8'); assert 'V2.7 광고 성과 대시보드' in t; assert 'Hook별 CTR' in t; assert 'https://' not in t; print('VERIFY_DB_GENERATOR_V2_7 PASS')"
if errorlevel 1 goto FAIL
