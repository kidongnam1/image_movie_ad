...
- TOP3 Hook reference IDs: `[{'hook_id': 28, 'source_id': 22, 'category': 'question'}, {'hook_id': 23, 'source_id': 22, 'category': 'problem'}, {'hook_id': 87, 'source_id': 29, 'category': 'curiosity'}]`
- TOP3 다양성: `{'unique_hook_ids': 3, 'unique_categories': 3}`

## TOP 3 Hooks
1. **왜 어떤 날은 serum를 발라도 사용감이 다르게 느껴질까요?** — 96.2점 (question) · DB hook=28 source=22
2. **serum을 써도 원하는 느낌이 안 나는 이유, 혹시 이것 때문일까요?** — 96.2점 (problem) · DB hook=23 source=22
3. **많은 분이 놓치는 serum 사용 포인트가 하나 있습니다.** — 93.7점 (curiosity) · DB hook=87 source=29

## 30초 대본

### 0-3s · HOOK
- 대사: 왜 어떤 날은 serum를 발라도 사용감이 다르게 느껴질까요?
- 자막: 왜 어떤 날은 serum를 발라도 사용감이 다르게 느껴질까요?
- 화면: serum 제품을 빠르게 클로즈업. 손에 들고 카메라 쪽으로 보여준다.

### 3-8s · PROBLEM
- 대사: serum는 제품 자체만큼 내 피부 상태와 바르는 순서, 사용량을 같이 보는 게 중요합니다.
- 자막: 제품 + 피부상태 + 사용순서
- 화면: 세안 후 피부와 제품을 번갈아 보여주는 짧은 컷.

### 8-17s · DEMO
- 대사: 제형 확인 → 소량 도포 → 사용감 체크. serum는 손등에서 먼저 제형을 확인하고 얼굴에는 소량씩 나눠 발라보세요.
- 자막: 제형 확인 → 소량 도포 → 사용감 체크
- 화면: 드로퍼/펌프에서 serum 한 방울. 매크로 제형 촬영 후 볼에 소량 도포.
...
- 자막: 제품 정보·사용법 확인
- 화면: serum 패키지와 제품 페이지를 연상시키는 깔끔한 엔드카드.
...
### Shot 1
```text
Vertical 9:16 premium Korean beauty short-form ad for a skincare serum.
Scene timing: 0-3s. Purpose: HOOK.
Visual action: serum 제품을 빠르게 클로즈업. 손에 들고 카메라 쪽으로 보여준다.
On-screen text concept: 왜 어떤 날은 serum를 발라도 사용감이 다르게 느껴질까요?
...
Visual action: 드로퍼/펌프에서 serum 한 방울. 매크로 제형 촬영 후 볼에 소량 도포.
...
Visual action: serum 패키지와 제품 페이지를 연상시키는 깔끔한 엔드카드.
